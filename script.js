...
function setCorrectActiveState(songId, versionType) {
    ...
    document.querySelectorAll('.swiper-slide').forEach(slide => {
        ...
        if (slideSongId === songId) {
            ...
            // Update the artist name in carousel
            const artistElement = slide.querySelector('.carousel-artist');
            if (artistElement) {
                const song = songs.find(s => s.id === songId);
                if (song) {
                    artistElement.textContent = getArtistForType(song, versionType);
                }
            }
        } else {
            slide.classList.remove('active');
...
function updateSelectedSongUI(songId, versionType) {
    ...
    requestAnimationFrame(() => {
        try {
            ...
            // Update swiper to show correct slide - FIX: Ensure swiper shows correct slide
...
function renderSongList() {
    ...
    filteredSongs.forEach((song, index) => {
        ...
        // FIXED: Version button click handler - UPDATED FOR CAROUSEL
            ...
            btn.addEventListener('click', (e) => {
                ...
                if (versionType !== currentFilterType) {
                    ...
                    setTimeout(() => {
                        ...
                        // Re-activate the current song - IMPORTANT FIX
                        const newFilteredIndex = filteredSongs.findIndex(s => s.id === songId);
                        if (newFilteredIndex >= 0 && swiper) {
                            setTimeout(() => {
                                swiper.slideToLoop(newFilteredIndex, 0, false);
                            }, 100);
                        }
                    ...
                    // If same filter, just update carousel slide
                    const slideIndex = filteredSongs.findIndex(s => s.id === songId);
                    if (slideIndex >= 0 && swiper) {
                        setTimeout(() => {
                            swiper.slideToLoop(slideIndex, 0, false);
                        }, 10);
                    }
        ...
        // FIXED: Lyrics button click - DON'T STOP PLAYING SONG
        songItem.querySelectorAll('.version-btn.lyrics-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                if (songIndex >= 0) {
                    ...
                    // Don't call setActiveSong() - just update UI without stopping playback
                    currentSongIndex = songIndex;
                    songs.forEach((s, i) => {
                        s.active = (i === songIndex);
                    });
                    
                    // Update UI without stopping playback
                    updateSelectedSongUI(songId, song.currentType);
                    
                    // Load and show lyrics WITHOUT stopping audio
...
function loadSong(index, autoPlay = true) {
    ...
    // FIX: Ensure carousel shows correct slide
    if (swiper && filteredSongs.length > 0) {
        ...
        if (songToActivate) {
            ...
            if (originalIndex >= 0) {
                ...
                if (swiper) {
                    ...
                    if (filteredIndex >= 0) {
                        setTimeout(() => {
                            swiper.slideToLoop(filteredIndex, 0, false);
                        }, 100);
                    }
                }
            }
...
function setupEventListeners() {
    ...
    // FIXED: Lyrics button - don't stop playback
    lyricsBtn.addEventListener('click', async () => {
        markUserInteraction();
        const currentSong = songs[currentSongIndex];
        await loadLyrics(currentSong.lyrics, currentSong, currentSong.currentType);
        lyricsModal.classList.add('active');
        // Don't stop playback - just show lyrics
    });
